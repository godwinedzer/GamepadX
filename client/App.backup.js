...
export default function App() {
  ...
  const makeStickGesture = (isLeft) =>
    Gesture.Pan()
      ...
      .onBegin(() => {
        ...
        scaleVal.value = withSpring(1.15, { damping: 14, stiffness: 130 });
      ...
      .onFinalize(() => {
        'worklet';
        const scaleVal = isLeft ? leftScale : rightScale;
        scaleVal.value = withSpring(1, { damping: 15, stiffness: 110 });

        if (isLeft) {
          leftX.value = withSpring(0, { damping: 15, stiffness: 120 });
          leftY.value = withSpring(0, { damping: 15, stiffness: 120 });
          lx.value = 0;
          ly.value = 0;
        } else {
          rightX.value = withSpring(0, { damping: 15, stiffness: 120 });
          rightY.value = withSpring(0, { damping: 15, stiffness: 120 });
          rx.value = 0;
          ry.value = 0;
        }
      });
  ...
  const ControlBtn = ({
    ...
    const btnAnimStyle = useAnimatedStyle(() => ({
      ...
      transform: [{ scale: active.value < 1 ? 0.93 : 1 }],
    }));
  ...
  const RapidFireBtn = () => {
    ...
    return (
      <RNGHTouchable onPress={toggle}>
        <View style={[styles.modeBtn, rapidFireOn ? styles.rapidBtnActive : styles.modeBtnInactive, { marginTop: 6 }]}>
          <Text style={styles.modeBtnText}>
  ...
  const handleConnect = async () => {
    setLoading(true);
    try {
      await AsyncStorage?.setItem('last_ip', ip).catch(() => null);
      resetInputState();
      networkRef.current = new GamepadNetwork(ip, 8888, 0);
      setConnected(true);
        ...
        <Animated.View style={[styles.loginRoot, loginStyle]}>
          <View style={styles.loginCard}>
            <Text style={styles.loginTitle}>
              ZAP<Text style={{ color: C.blue }}>PRO</Text>
            </Text>
      ...
      <Animated.View style={[styles.padContainer, padContainerStyle]}>
        ...
        <View pointerEvents="box-none" style={styles.gameArea}>
          ...
          <View style={styles.col3}>
            ...
            <Animated.Text style={[styles.zapLogo, logoStyle]}>ZAP</Animated.Text>
...
// STYLESHEET
// ─────────────────────────────────────────────────────────────
const COL_SIDE_W = Math.round(SCREEN_W * 0.22);   // ~22% each side
const COL_STICK_W = Math.round(SCREEN_W * 0.18);  // ~18% stick columns
const STICK_SIZE  = Math.round(SCREEN_H * 0.55);  // stick circle diameter
const HANDLE_SIZE = Math.round(STICK_SIZE * 0.52);
const DPAD_BTN    = Math.round(SCREEN_H * 0.12);
const ACTION_BTN  = Math.round(SCREEN_H * 0.14);
const SHOULDER_W  = Math.round(COL_SIDE_W * 0.44);
const SHOULDER_H  = Math.round(SCREEN_H * 0.13);
...
const styles = StyleSheet.create({
  ...
  // 5-column row filling the whole screen
  gameArea: {
    flex: 1,
    flexDirection: 'row',
    alignItems: 'stretch',
  ...
  col4: {
    width: COL_STICK_W,
    justifyContent: 'flex-end',     // right stick sits near bottom
    alignItems: 'center',
    paddingBottom: Math.round(SCREEN_H * 0.06),
  },
  col5: {
    width: COL_SIDE_W,
    justifyContent: 'space-between',
